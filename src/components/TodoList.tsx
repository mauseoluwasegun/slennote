...
export const TodoList = forwardRef<TodoListRef, TodoListProps>(
  (
    ...
    const voiceInputRef = React.useRef<VoiceInputRef | null>(null);
    const [voiceError, setVoiceError] = React.useState<string | null>(null);
    const interimTranscriptRef = React.useRef<string>("");
    const baseContentRef = React.useRef<string>("");
    ...
    const handleVoiceError = (error: string) => {
      ...
      const cleanBase = baseContentRef.current.replace(/ \(Listening\.\.\.\)$/, "").replace(/ \(Transcribing\.\.\.\)$/, "");
      setNewTodoContent(cleanBase);
      baseContentRef.current = cleanBase;
    ...
    const handleVoiceStart = () => {
      // Initialize base content and focus textarea when voice recording starts
      baseContentRef.current = newTodoContent;
      setNewTodoContent(newTodoContent + " (Listening...)");
    ...
    const handleProcessingStart = () => {
      ...
      const cleanBase = baseContentRef.current.replace(/ \(Listening\.\.\.\)$/, "");
      baseContentRef.current = cleanBase; // Update base ref to avoid double suffix
      setNewTodoContent(cleanBase + " (Transcribing...)");
    };
