...
export function TodoItem({
  ...
  const breakdownTodo = useAction(api.aiBreakdown.breakdownTodoWithAI);
  ...
  const handleAIBreakdown = async () => {
    ...
    try {
      const result = await breakdownTodo({ todoContent: content });

      // Create subtasks from the result
      ...
      console.error("Error breaking down todo:", error);
      setAiError("Failed to generate subtasks");
          ...
          {showMenu &&
            (isArchived ? (
              ...
              <div className="menu-dropdown" ref={menuDropdownRef}>
                ...
                {!completed && !isBacklogView && (
                  <div
                    className={`menu-item ${isAIBreakingDown ? 'disabled' : ''}`}
                    onClick={isAIBreakingDown ? undefined : handleAIBreakdown}
                    style={{ opacity: isAIBreakingDown ? 0.6 : 1 }}
                  >
                    {isAIBreakingDown ? '⏳ Breaking down...' : '✨ Break down with AI'}
                  </div>
                )}
