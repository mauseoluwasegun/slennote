...
export const updateShareSlug = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Construct share URL
    const baseUrl = process.env.CONVEX_SITE_URL || "https://slennote.vercel.app/";
    const shareUrl = `${baseUrl}/share/${args.newSlug}`;
...
// Internal query to get notes by IDs for AI chat actions (no auth check - called from actions)
export const getNotesByIdsInternal = internalQuery({
  args: {
    noteIds: v.array(v.id("fullPageNotes")),
    userId: v.string(),
  },
...
// Internal query to get all recent notes for a user (for AI context)
  ...
  handler: async (ctx, args) => {
    ...
    return notes
      ...
      .map((note) => ({
        _id: note._id,
        title: note.title || "Untitled",
        content: note.content.slice(0, 500), // Truncate for list preview
      }));
  },
});
